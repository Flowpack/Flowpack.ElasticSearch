...
class BackendMappingBuilder {
	 ...
	 * @return \TYPO3\ElasticSearch\Mapping\MappingCollection<\TYPO3\ElasticSearch\Domain\Model\Mapping>
	 */
	public function buildMappingInformation() {
		if (!$this->client instanceof Model\Client) {
			throw new \TYPO3\ElasticSearch\Exception('No client was given for mapping retrieval. Set a client BackendMappingBuilder->setClient().', 1339678111);
		}
	...
	public function getIndicesWithoutTypeInformation() {
		if ($this->indicesWithoutTypeInformation === NULL) {
			throw new \TYPO3\ElasticSearch\Exception('For getting the indices having no mapping information attached, BackendMappingBuilder->buildMappingInformation() has to be run first.', 1339751812);
		}
